...
function VerifyDigitalCode() {
  const [emailforgotPassword] = useState(localStorage.getItem("emailforgotPassword"));
  ...
  const handleResendPass = async () => {
    await http.get(`/auth/forgot-password?email=${emailforgotPassword}`)
  }
  return (
    <AuthLayout>
        ...
        <div className="verify-main">
          ...
          <form onSubmit={formik.handleSubmit} className="verify-form">
            <div className="verify-form-group">
              <div className="verify-text-form">Enter the code we’ve just send you to change your password: <span>{emailforgotPassword}</span></div>
